...
float
calculatedScoreC(double voicePitch, double stdPitch, int scoreLevel, int scoreCompensationOffset) {
    if (voicePitch <= 0) {
        return 0;
    }
    if (stdPitch <= 0) {
    ...
    float match = 1 - (float) scoreLevel / 100 * fabs(voiceTone - stdTone) +
                  (float) scoreCompensationOffset / 100;
